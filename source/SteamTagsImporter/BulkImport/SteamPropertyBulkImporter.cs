...
public class SteamPropertyBulkImporter : BulkGamePropertyAssigner<SteamProperty, GamePropertyImportViewModel>
{
    ...
    {
        yield return new(MetadataProviderName, game => game.Url, ContainsUrl);
        yield return new("Discussions", game => $"https://steamcommunity.com/app/{game.Id}/discussions/", ContainsUrl) { Checked = false };
        yield return new("Guides", game => $"https://steamcommunity.com/app/{game.Id}/guides/", ContainsUrl) { Checked = false };
        ...
        switch (searchItem.Value)
        {
            case "22": //Achievements
                yield return new("Achievements", game => $"https://steamcommunity.com/stats/{game.Id}/achievements", ContainsUrl);
                break;
            case "29": //Cards
                yield return new("Badge Progress", game => $"https://steamcommunity.com/my/gamecards/{game.Id}", ContainsUrl);
                yield return new("Points Shop", game => $"https://store.steampowered.com/points/shop/app/{game.Id}/", ContainsUrl);
                break;
            case "30": //Workshop
                yield return new("Workshop", game => $"https://steamcommunity.com/app/{game.Id}/workshop/", ContainsUrl);
                break;
        }
    }

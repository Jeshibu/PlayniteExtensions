...
namespace BigFishLibrary
{
    public class BigFishOnlineLibraryScraper
    {
        ...
        public IEnumerable<Product> GetGames()
        {
            ...
            do
            {
                var response = downloader.DownloadString(GetLibraryUrl(page), customHeaders: new Dictionary<string, string> { { "authorization", $"Bearer {token}" } });
        ...
        private string GetToken()
        {
            ...
            var scriptTask = ExecuteJavaScriptOnPage(url, script, maxAttempts: 2, millisecondsDelayAfterPageLoad: 2000);
            scriptTask.Wait();
        ...
        private async Task<object> ExecuteJavaScriptOnPage(string url, string script, int maxAttempts = 1, int millisecondsDelayAfterPageLoad = 0)
        {
            using (var webView = playniteApi.WebViews.CreateOffscreenView())
            {
                for (int attempt = 0; attempt < maxAttempts; attempt++)
                {
                    ...
                    //if (millisecondsDelayAfterPageLoad > 0)
                    //    await Task.Delay(millisecondsDelayAfterPageLoad);
